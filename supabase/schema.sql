...
-- app_users: cada usuário enxerga colegas da própria empresa.
create policy app_users_select_same_company on public.app_users
for select
using (company_id = public.current_company_id());

-- app_users: somente master da empresa pode inserir/atualizar/deletar colegas.
create policy app_users_manage_master on public.app_users
for all
using (public.is_master_of_company(company_id))
with check (public.is_master_of_company(company_id));

-- Accounts/Categories/Transactions/Onboarding por company_id
create policy accounts_same_company on public.accounts
for all using (company_id = public.current_company_id())
with check (company_id = public.current_company_id());

create policy categories_same_company on public.categories
for all using (company_id = public.current_company_id())
with check (company_id = public.current_company_id());

create policy transactions_same_company on public.transactions
for all using (company_id = public.current_company_id())
with check (company_id = public.current_company_id());

create policy onboarding_same_company on public.company_onboarding
for all using (company_id = public.current_company_id())
with check (company_id = public.current_company_id());
